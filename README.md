...
- Native source-system grant previews (`get_source_system_access`, alias `user_object_access`) — Redshift / Snowflake / Tableau, not catalog ACLs
...
| **Native DB/BI access** | Use **`get_source_system_access`** (alias **`user_object_access`**) for Redshift/Snowflake/Tableau grants — not OvalEdge catalog ACLs. |
...
- `get_source_system_access`, `user_object_access` (Redshift / Snowflake / Tableau grant previews)
