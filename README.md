...
Git hooks (**ruff** + **mypy** + full **pytest** on each **commit**; same checks again on **push**) are installed automatically when you run `./scripts/setup_local_mcp.sh` in a git clone. To install or refresh hooks only:
