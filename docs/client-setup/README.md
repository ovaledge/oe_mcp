...
**Quick routing:** organizational policy/playbooks → `lookup_datastory`; physical datasets → `search_catalog_assets`; OvalEdge UI/features → `search_platform_docs`; native Redshift/Snowflake/Tableau grants → `get_source_system_access` (alias `user_object_access`).
