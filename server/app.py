...
_MCP_SERVER_INSTRUCTIONS = (
    ...
    "Who-has-access without native/DAM or catalog ACL/OE security signals: invoke prompt "
    "resolve_object_access, present the 1/2 choice, and call no tools "
    "(including search_catalog_assets) until the user replies. "
    ...
    "Who-has-access with native/DAM signals: source_system_access with "
    "access_intent_confirmed=native (or user_question carrying those signals); "
    "never fall back to catalog search for RDAM. "
    "Who-has-access with catalog ACL/OE security/ACL signals: get_user_object_access with "
    "access_intent_confirmed=catalog_acl (or user_question carrying those signals). "
    "After user picks 1 (native): source_system_access with access_intent_confirmed=native. "
