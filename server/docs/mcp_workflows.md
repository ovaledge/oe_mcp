...
7. **Field validations:** OvalEdge enforces lookup `validations` and `fieldData.options` on create. If a value is rejected, show the API error and ask for a corrected value. Do not invent replacement copy for character limits, dropdowns, URLs, or dates.
8. Re-call with `ticket_template_id`, `object_id` (one id, a list, or comma-separated ids when Select Table `allowMultiple` is true), `summary`, and any user-supplied `ticket_fields` for **confirm_create**.
