...
def register(mcp: FastMCP) -> None:
    ...
    @mcp.prompt()
    def native_source_access(question: str) -> list[Message]:
        """
        Native Redshift/Snowflake/Tableau grants (RDAM harvest), not OvalEdge catalog ACLs.

        Trigger: "Who can SELECT on BUSINESS.BANKING.ALERTS in Snowflake?"
                 "What tables can VINAY query in Redshift?"
        """
        text = (
            f"Answer this native source-system access question: '{question}'\n\n"
            f"Use {TOOL_GET_SOURCE_SYSTEM_ACCESS} only (not catalog ACL tools). "
            f"Pass connection_id when known — the server enforces Instance/Connector DAA.\n\n"
            f"Steps:\n"
            f"1. Infer source_system: redshift, snowflake, or tableau\n"
            f"2. user_to_objects → username; object_to_users → object_path\n"
            f"3. object_path patterns (Redshift/Snowflake):\n"
            f"   - dbName only (e.g. BUSINESS)\n"
            f"   - connectionName.dbName (e.g. snowflake.BUSINESS) "
f"when multiple connections exist\n"
            f"   - dbName.schema.table for a full table (e.g. BUSINESS.BANKING.ALERTS)\n"
            f"   - Short table names (e.g. ALERTS) are partial — if ambiguous, use matchCandidates "
            f"or resolve_all_matches=true, or pass connection_id\n"
            f"4. Present grants with principal_name, privileges, grant_mechanism, "
            f"contributing_role, and object_path\n"
            f"5. If RDAM no-access error, explain caller needs Connector or Instance DAA\n"
            f"6. Summarize by role and privilege; note if zero grants (common for "
            f"INFORMATION_SCHEMA-only catalog hits)\n\n"
            f"Reference:\n{MCP_OBJECT_PATH_FORMATS_DOC}"
        )
        return [Message(text)]
