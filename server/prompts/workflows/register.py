...
def register(mcp: FastMCP) -> None:
    ...
    def data_discovery(query: str) -> list[Message]:
        ...
        text = (
            ...
            f"(owner, steward, connection, or an inferred object_type) if scope is unclear\n"
    ...
    def assign_governance_roles(
        ...
        text = (
            ...
            f"Steps:\n"
            f"1. If the target sounds like a DQ rule, call {TOOL_DQ_RULE_ADVISOR} step=lookup first; "
    ...
    def assess_cde_dq_coverage(scope: str) -> list[Message]:
        ...
        text = (
            ...
            f"{TOOL_DQ_RULE_ADVISOR}(step=assess, discover_cde_columns=true) only when listing all CDE columns\n"
            ...
            f"3b. If the user rejects candidates, re-call {TOOL_DQ_RULE_ADVISOR} step=assess with "
            ...
            f"({TOOL_DQ_RULE_ADVISOR} step=generate_query)\n"
            f"4. Use {TOOL_DQ_RULE_ADVISOR} step=lookup when the user names an existing rule; do not use "
            f"{TOOL_ASSET_EXPLORER} for dqrule objects\n"
            f"5. Only after explicit user approval for writes: {TOOL_DQ_RULE_MANAGER} step=associate "
            f"for a known data quality rule id, or {TOOL_DQ_RULE_MANAGER} step=create_standard "
    ...
    def create_custom_sql_dq_workflow(user_intent: str) -> list[Message]:
        ...
        text = (
            ...
            f"2. Call {TOOL_DQ_RULE_ADVISOR} step=assess with explicit objects for the named assets "
            ...
            f"{TOOL_DQ_RULE_MANAGER}(step=create_standard, prefer_existing_rule=false) with confirm gate\n"
            ...
            f"6. For custom_sql workflow: {TOOL_DQ_RULE_ADVISOR} step=generate_query → use "
            f"connection_id/schema_id from formattedResponse or data.context → "
            f"{TOOL_DQ_RULE_ADVISOR} step=validate_query (confirm gate) → {TOOL_DQ_RULE_MANAGER} step=create_custom_sql "
            ...
            f"7. Ad-hoc rule lookup: {TOOL_DQ_RULE_ADVISOR} step=lookup (not catalog search)"
        )
        return [Message(text)]
