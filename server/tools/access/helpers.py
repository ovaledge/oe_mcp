...
_DESC_ACCESS_EXPLORER = classify_tool_desc(
    ...
    "browse). Named FQN/object_path without connection_id resolve via `asset_explorer`, "
    "then always DAM (`resolveSourceSystemAccess`) even if catalog is empty/ambiguous. "
    "`object_path` + `connection_id` skips catalog resolve. `object_id` is resolved in Java. "
