...
_DESC_ACCESS_EXPLORER = classify_tool_desc(
    ...
    "browse | role_to_users | group_to_users | user_to_roles | user_to_groups). Named objects: "
    "`asset_explorer` this tool fills object_id, object_type, connection_id, FQN/object_path, "
    "object_name, then DAM API. "
    ...
    "**role_to_users:** role name via object_path or object_name (no object_type). "
    "**group_to_users:** group name via object_path or object_name (Redshift/Tableau; "
    "not Snowflake). "
    "**user_to_roles:** username required; lists roles assigned to the user (no object_type). "
    "**user_to_groups:** username required; lists groups (Redshift/Tableau; not Snowflake). "
