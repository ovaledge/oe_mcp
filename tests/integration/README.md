...
| Area | Covered |
|------|---------|
| `asset_explorer` | POST `/api/v1/mcp/asset-explorer` (never GET); `context_query` ranking, multi-term search, owner/steward filters, connection and server-type scoping, CDE flag, exact glossary/tag filters, nested `filters` well-formedness (`rating` / `dqIndex` / `popularity` / `nullDensity` / `density` / `rowCount` / `columnCount` ranges including `eq`, `createdDate` `{from,to}`), `search.sort` `{field,direction}`, pagination disjointness, page-size cap, `oecolumn` type filter, tag hierarchy, no-match empties, navigation links |
